...
Repository history allows the unpinned clone used during this release window to be reconstructed with high confidence: `godaddy/libenclaveapp` is a renamed repository URL that GitHub currently redirects to `godaddy/hardware-enclave`, so the identical commit namespace is expected rather than a copy/paste coincidence. The canonical repository's `main` tip immediately before the run was `2fd3cf2594f8b9446004b73feef7ef2dd7954bdf`, and no commits landed on `main` between release-run start and release publication. However, the workflow itself cloned the unpinned legacy repository name and did not enforce that SHA. The machine-readable pin records this separately as an **inferred** build input with `enforced_by_workflow: false`; consumers must not upgrade that inference into a cryptographic provenance claim.
